...
<div class="auth-page">
  <div class="auth-card">
    <div class="auth-header">
      ...
      <p>Inscrivez-vous via Keycloak pour accéder à l'application.</p>
    </div>
    <form (ngSubmit)="onSubmit()" class="auth-form">
      @if (error()) { <div class="alert alert-error">{{ error() }}</div> }
      <button type="submit" class="btn btn-primary" [disabled]="loading()">
        @if (loading()) { Création en cours... } @else { S'inscrire avec Keycloak }
      </button>
    </form>
    <p class="auth-footer">
...
export class RegisterComponent {
  ...
  async onSubmit() {
    ...
    try {
      ...
      this.error.set(message ?? "Erreur lors de l'inscription");
    } finally {
